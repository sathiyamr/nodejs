...
const { adminAuth, userAuth } = require("./middlewares/auth_tutorial");
...
app.get("/users", (req, res, next) => {
  try {
    ...
    res.status(500).send("hey sry someting went wrong !!!!");
  }
...
app.use("/", (err, req, res, next) => {
  if (err) {
    ...
    res.send(
      "Works for ALL HTTP methods, Matches prefix, this is default one "
    );
  }
});
